...
# Set to true to seed permissions, roles, and (in development/test only) default users. Never enable in production with default passwords.
SEED_DATABASE = false # default false
```
...
- **Unit tests** (error middleware, validation schemas) run without any environment setup.
- **Integration tests** (auth API) require a `.env` file with `JWT_ACCESS_TOKEN_SECRET_PRIVATE` and `JWT_ACCESS_TOKEN_SECRET_PUBLIC` (base64-encoded RS256 keys). If these are not set, the auth integration suite is skipped.
- Seed data is gated by `SEED_DATABASE`. When `SEED_DATABASE=true`, the app seeds permissions and roles on startup; in development/test it also seeds default users. Do not enable in production with default passwords.
...
`GET api/v1/auth/current` - Current user (minimal)\
`GET api/v1/auth/me` - Profile (with roles)\
